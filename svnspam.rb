...
class CommentHandler < LineConsumer
  ...
  def teardown
    unless @comment == @lastComment
      println("<pre style=\"padding:4px;border:1px dashed #000000;background-color:#ffffdd;\">")
...
class UnifiedDiffColouriser < LineConsumer
  ...
  def consume(line)
    ...
    if initial != @currentState
      ...
      if initial=="+" && @currentState=="-" && @lineJustDeleted!=nil
        ...
        if prefixLen==1 && suffixLen==0 || deleteInfixSize<=0 || oversize_change
          ...
          print("<span id=\"removedchars\" style=\"background-color:#ff9999;font-weight:bolder;\">")
        ...
        if superlong_line
          println("<strong style=\"color:red;\">[...]</strong>")
        else
          println("")
        end
        @lineJustDeleted = nil
      ...
      if prefixLen==1 && suffixLen==0 || addInfixSize<=0 || oversize_change
        ...
        "<span id=\"addedchars\" style=\"background-color:#99ff99;font-weight:bolder;\">" +
    ...
    if initial=="+"
      $task_keywords.each do |task|
        if line =~ /\b(#{task}\b.*)/
          $task_list << $1
          encoded.sub!(/\b#{task}\b/, "<span style=\"background-color:#ffff00;\">#{task}</span>")
  ...
  def teardown
    ...
    if @truncatedLineCount>0
      println("<strong style=\"color:red;\" title=\"#{@truncatedLineCount} lines truncated at column #{$maxDiffLineLength}\">[Note: Some over-long lines of diff output only partialy shown]</strong>")
    end
  end

  # start the diff output, using the given lines as the 'preamble' bit
  def start_output(*lines)
    println("<hr style=\"border-width:0px;height:2px;background:black;\"/><a name=\"file#{$fileEntries.size}\" /><div style=\"border:1px solid #eeeeee;margin-top:1em;margin-bottom:1em;\">")
    case $file.type
      when "A"
        print("<span style=\"font-family:monospace; float:right;background-color:#ddffdd;\" id=\"added\" >")
        print($frontend.path($file.basedir, $file.tag))
        println("</span><br />")
        println("<div style=\"margin-bottom:.5em;background-color:#ddffdd;\" id=\"added\" ><big><b>#{htmlEncode($file.file)}</b></big> <small id=\"info\" style=\"color:#888888;\">added at #{$frontend.version($file.path,$file.toVer)}</small></div>")
      when "R"
        print("<span style=\"font-family:monospace; float:right;background-color:#ffdddd;\" id=\"removed\">")
        print($frontend.path($file.basedir, $file.tag))
        println("</span><br />")
        println("<div style=\"margin-bottom:.5em;background-color:#ffdddd;\" id=\"removed\" ><big><b>#{htmlEncode($file.file)}</b></big> <small id=\"info\" style=\"color:#888888;\">removed after #{$frontend.version($file.path,$file.fromVer)}</small></div>")
      when "M"
        print("<span style=\"font-family:monospace; float:right;\">")
        print($frontend.path($file.basedir, $file.tag))
        println("</span><br />")
        println("<div style=\"margin-bottom:.5em;\"><big><b>#{htmlEncode($file.file)}</b></big> <small id=\"info\" style=\"color:#888888;\">#{$frontend.version($file.path,$file.fromVer)} #{$frontend.diff($file)} #{$frontend.version($file.path,$file.toVer)}</small></div>")
    end
    print("<pre style=\"margin:0;color:#888888;\"><small id=\"info\">")
    lines.each do |line|
  ...
  def teardown
    if @lookahead == nil
      ...
      if $file.wants_diff_in_mail?
        if @stats.diffLines > $maxLinesPerDiff
          println("</pre>")
          println("<strong style=\"color:red;\">[truncated at #{$maxLinesPerDiff} lines; #{@stats.diffLines-$maxLinesPerDiff} more skipped]</strong>")
        else
          @colour.teardown
        end
...
HEAD
  ...
  unless ($problemHeaders.empty?)
    mail.puts("<strong style=\"color:red;\">Bad header format in '#{$config}':<ul>")
  ...
  $fileEntries.each do |file|
    unless file.repository == last_repository
      ...
      mail.print("<tr style=\"border-bottom-width:1px;border-bottom-style:solid;\"><td style=\"padding:0;padding-top:.2em;\"  colspan=\"#{last_repository.has_multiple_tags ? 5 : 4}\">")
      ...
      if last_repository.trunk_only?
        mail.print("<span id=\"info\" style=\"color:#888888;\"> on MAIN</span>")
      else
        mail.print(" on ")
        tagCount = 0
        last_repository.each_tag do |tag|
          ...
          mail.print tag ? htmlEncode(tag) : "<span id=\"info\" style=\"color:#888888;\">MAIN</span>"
        end
      end
      mail.puts("</td></tr>")
    end
    file_count += 1
    if (file_count%2==0)
      mail.print("<tr style=\"background-color:#eeeeee\">")
    else
      mail.print("<tr>")
    end
    if file.addition?
    ...
    if file.addition?
      name = "<span id=\"added\" style=\"background-color:#ddffdd;background-color:#ccf7cc;\">#{name}</span>"
    elsif file.removal?
      name = "<span id=\"removed\" style=\"background-color:#f7cccc;\">#{name}</span>"
    end
    if file.has_diff?
      mail.print("<td style=\"padding-left:.3em;padding-right:.3em;\" ><tt>#{prefix}<a href=\"#file#{file_count}\">#{name}</a></tt></td>")
    else
      mail.print("<td style=\"padding-left:.3em;padding-right:.3em;\" ><tt>#{prefix}#{name}</tt></td>")
    end
    if file.isEmpty
      mail.print("<td style=\"padding-left:.3em;padding-right:.3em;color:#888888;\"  colspan=\"2\" align=\"center\"><small id=\"info\">[empty]</small></td>")
    elsif file.isBinary
      mail.print("<td style=\"padding-left:.3em;padding-right:.3em;\"  colspan=\"2\" align=\"center\"><small id=\"info\" style=\"color:#888888;\">[binary]</small></td>")
    else
      if file.lineAdditions>0
        totalLinesAdded += file.lineAdditions
        mail.print("<td style=\"padding-left:.3em;padding-right:.3em;background-color:#ddffdd;background-color:#ccf7cc;\"  align=\"right\" id=\"added\">+#{file.lineAdditions}</td>")
      else
        mail.print("<td style=\"padding-left:.3em;padding-right:.3em;\" ></td>")
      end
      if file.lineRemovals>0
        totalLinesRemoved += file.lineRemovals
        mail.print("<td style=\"padding-left:.3em;padding-right:.3em;background-color:#f7cccc;\"  align=\"right\" id=\"removed\">-#{file.lineRemovals}</td>")
      else
        mail.print("<td style=\"padding-left:.3em;padding-right:.3em;\" ></td>")
      end
    end
    if last_repository.has_multiple_tags
      if file.tag
        mail.print("<td style=\"padding-left:.3em;padding-right:.3em;\" >#{htmlEncode(file.tag)}</td>")
      else
        mail.print("<td style=\"padding-left:.3em;padding-right:.3em;color:#888888;\" ><span id=\"info\">MAIN</span></td>")
      end
    elsif haveTags
      mail.print("<td style=\"padding-left:.3em;padding-right:.3em;\" ></td>")
    end
    if file.addition?
      mail.print("<td style=\"padding-left:.3em;padding-right:.3em;\"  nowrap=\"nowrap\" align=\"right\">added #{$frontend.version(file.path,file.toVer)}</td>")
    elsif file.removal?
      mail.print("<td style=\"padding-left:.3em;padding-right:.3em;\"  nowrap=\"nowrap\">#{$frontend.version(file.path,file.fromVer)} removed</td>")
    elsif file.modification?
      mail.print("<td style=\"padding-left:.3em;padding-right:.3em;\"  nowrap=\"nowrap\" align=\"center\">#{$frontend.version(file.path,file.fromVer)} #{$frontend.diff(file)} #{$frontend.version(file.path,file.toVer)}</td>")
    end
  ...
  if $fileEntries.size>1 && (totalLinesAdded+totalLinesRemoved)>0
    ...
    mail.print("<tr><td style=\"padding-left:.3em;padding-right:.3em;\" ></td>")
    if totalLinesAdded>0
      mail.print("<td style=\"padding-left:.3em;padding-right:.3em;background-color:#ddffdd;background-color:#ccf7cc;\"  align=\"right\" id=\"added\" >+#{totalLinesAdded}</td>")
    else
      mail.print("<td style=\"padding-left:.3em;padding-right:.3em;\" ></td>")
    end
    if totalLinesRemoved>0
      mail.print("<td style=\"padding-left:.3em;padding-right:.3em;background-color:#f7cccc;\"  align=\"right\" id=\"removed\">-#{totalLinesRemoved}</td>")
    else
      mail.print("<td style=\"padding-left:.3em;padding-right:.3em;\" ></td>")
    end
    mail.print("<td style=\"padding-left:.3em;padding-right:.3em;\" ></td>") if haveTags
    mail.puts("<td style=\"padding-left:.3em;padding-right:.3em;\" ></td></tr>")
  end
  ...
  if totalFilesChanged > 1
    mail.print("<small id=\"info\" style=\"color:#888888;\">")
    changeKind = 0
    if filesAdded>0
  ...
  if $task_list.size > 0
    task_count = 0
    mail.puts("<div style=\"padding:4px;border:1px dashed #000000;margin-top:1em;\"><ul style=\"margin-top:0;margin-bottom:0;\">")
  ...
  if $diff_output_limiter.choose_to_limit?
    mail.puts("<p><strong style=\"color:red;\">[Reached #{$diff_output_limiter.written_count} bytes of diffs.")
